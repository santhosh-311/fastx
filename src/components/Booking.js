...
const Booking=()=>{
    ...
    useEffect(() => {
        ...
        console.log("Fetching mybookings...");
        axios.get(`http://localhost:8084/booking/user/mybookings/${userDetails.userName}`, {
      ...
      const refreshBookings = async () => {
        try {
            const response = await axios.get(
                `http://localhost:8084/booking/user/mybookings/${userDetails.userName}`,
                { headers: { Authorization: `Bearer ${token}` } }
            );
